...
& $Bison -d -y --debug --verbose 22101047.y
...
if (Test-Path input.c) {
    ...
    # Execute parser passing file via stdin alternative: direct arg or pipeline
    # Prefer direct file argument if your parser reads argv; fallback to pipe.
    if (Test-Path $OutExe) {
        & .\\$OutExe input.c 2>$null
        if ($LASTEXITCODE -ne 0) {
            Write-Host "Direct execution returned code $LASTEXITCODE, trying stdin pipe" -ForegroundColor DarkYellow
